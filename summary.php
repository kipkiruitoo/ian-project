...
require('db.php');
    $id=$_SESSION['id'];
    $query=mysqli_query($mysqli,"select * from reservation left join team on reservation.team_id=team.team_id natural join combo where rid='$id'")or die(mysqli_error($mysqli));
      $row=mysqli_fetch_array($query);
        $rcode=$row['r_code'];
        $first=$row['r_first'];
        $last=$row['r_last'];
        $mysqlitact=$row['r_mysqlitact'];
...
<?php
    ...
    $query1=mysqli_query($mysqli,"select * from combo_details natural join menu where combo_id='$cid'")or die(mysqli_error($mysqli));
